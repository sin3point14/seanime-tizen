...
export function Shell({ route, navigate, children }: { route: Route; navigate: (route: Route) => void; children: React.ReactNode }) {
  ...
  return <FocusContext.Provider value={focusKey}><div ref={ref} className="app-shell">
    <header className="topbar">
      ...
      <nav>
        <Focusable focusKey="NAV_HOME" className={route.screen === "home" ? "active" : ""} onEnter={() => navigate({ screen: "home" })}>Home</Focusable>
        <Focusable focusKey="NAV_SEARCH" className={route.screen === "search" ? "active" : ""} onEnter={() => navigate({ screen: "search" })}>Search</Focusable>
        <Focusable focusKey="NAV_SETTINGS" className={route.screen === "settings" ? "active" : ""} onEnter={() => navigate({ screen: "settings" })}>Settings</Focusable>
      </nav>
